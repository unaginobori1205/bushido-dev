# Authentic Japan — 13-Day Model Plan & Estimate
### Prepared by BUSHIDO JAPAN for Yasmin Sabbag (Travel Agency, Colombia)

> Date: 2026-06-20 · Prepared by Kensuke Ueoka, BUSHIDO JAPAN
> Trade proposal (Land Operator → Travel Agency) · **Draft — pending group size & budget**
> Scope of our quote: **Japan land arrangements only (Osaka IN → Narita OUT).** International flights & Seoul stay are arranged on your side (we can coordinate if helpful).

---

## 1. Your Group at a Glance

| | |
|---|---|
| Travel dates | **Oct 6 – 19, 2026** (approx. 13 nights, incl. Seoul) |
| Route logic | Seoul first (cooler) → enter Japan via Osaka → exit Tokyo → Seoul (cheaper international fares) |
| Japan departure | **Narita (NRT)** preferred; Haneda (HND) also fine |
| Interests | Must-see icons + our signature cultural experiences · food · Mt. Fuji · hot springs · nightlife · hidden local spots · samurai |
| Group | **3 travelers — mother + two ~18-year-old daughters, sharing one room (triple)** |

---

## 2. Why BUSHIDO JAPAN

We are a Nagoya-based land operator specializing in **authentic, hands-on Japanese culture** — tea ceremony, calligraphy, kyudo (archery), kimono and samurai/bushido — delivered with real masters, not tourist shows. By placing a **2-night Nagoya cultural core** on the natural Kyoto–Tokyo corridor, your client gets an itinerary **no other agency can copy**.

---

## 3. Route Overview (13 nights)

**Seoul (2)** → **Osaka (2)** → **Kyoto (2)** *(via Nara)* → **Nagoya (2)** → **Hakone / Mt. Fuji (1)** → **Tokyo (3)** → Seoul → home
*Japan land portion handled by BUSHIDO = 10 nights (Osaka 2 · Kyoto 2 · Nagoya 2 · Hakone 1 · Tokyo 3).*

---

## 4. Day-by-Day Model Plan

| Date | Base | Highlights |
|---|---|---|
| Oct 6 | → Seoul | Arrival (your side) |
| Oct 7 | Seoul | Seoul sightseeing (your side) |
| **Oct 8** | **Osaka IN** | Arrive Kansai; check-in Namba/Umeda |
| **Oct 9** | Osaka | Osaka Castle · Dotonbori street-food tour (*kuidaore*) · Shinsekai · **nightlife** |
| **Oct 10** | → Nara → Kyoto | Todai-ji Great Buddha · Nara Park deer · Kasuga Taisha → Kyoto |
| **Oct 11** | Kyoto | Fushimi Inari · Kinkaku-ji · Arashiyama bamboo · Kiyomizu-dera · Gion · **samurai/sword experience** or maiko dinner |
| **Oct 12** | → Nagoya | **CULTURE DAY 1:** kimono dressing · Nagoya Castle · Atsuta Shrine · *unagi hitsumabushi* dinner |
| **Oct 13** | Nagoya / Inuyama / Arimatsu | **CULTURE DAY 2:** kyudo + calligraphy · hand-made miso-nikomi udon · Arimatsu shibori tie-dye · Nishikawa-ryu Japanese dance · Inuyama Castle keep |
| **Oct 14** | → Hakone / Mt. Fuji | Mt. Fuji views · Lake Ashi cruise · Owakudani · **onsen ryokan (hot springs)** |
| **Oct 15** | → Tokyo | Hakone Shrine torii · ropeway → Tokyo |
| **Oct 16** | Tokyo | Senso-ji (Asakusa) · Meiji Shrine · Shibuya Crossing · **teamLab** |
| **Oct 17** | Tokyo | Free day · **hidden local** Yanaka/Nezu · Toyosu Market · Shinjuku Golden Gai **nightlife** |
| **Oct 18** | → Seoul | Private transfer to Narita; depart |
| Oct 19 | Seoul → home | Return (your side) |

---

## 5. Signature: Nagoya / Aichi Experience Menu (our exclusive core)

> Real per-person rates. We build the 2 days by combining items from this menu — all guided in English with authentic masters.

| Experience | Price / person |
|---|---|
| Kyudo (archery) + Calligraphy — set | **¥20,000** |
| Hand-made Miso-nikomi Udon (Nagoya specialty) | **¥10,000** |
| Nishikawa-ryu Japanese dance (Nagoya-born school) | **¥20,000** |
| Arimatsu Shibori tie-dye craft | **¥4,000–5,000** (TBC) |
| Unagi / *hitsumabushi* signature meal (with our own "Unaginobori") | **¥6,000–8,000** (TBC) |
| Kimono dressing · Tea ceremony · Castle entries | quoted separately |

> **Standard 2-day set example ≈ ¥61,500/person** (kimono, tea ceremony and entry fees additional).

---

## 6. Hotel Line-Up (3 grades, twin share)

| City | Value (3★) | Standard (4★) | Luxury (5★) |
|---|---|---|---|
| Osaka | Hotel Hankyu Respire Osaka | Swissôtel Nankai Osaka | The St. Regis Osaka |
| Kyoto | Mitsui Garden Kyoto Station | Hotel Granvia Kyoto | The Ritz-Carlton, Kyoto |
| Nagoya | Meitetsu Grand Hotel | Nagoya Marriott Associa | Espacio Nagoya Castle / The Tower Hotel Nagoya |
| Hakone/Fuji | Onsen hotel (Fuji area) | Hakone Yutowa *(ryokan, half-board)* | Gora Kadan / Hoshinoya Fuji |
| Tokyo | Shibuya Tobu Hotel | Shibuya Excel Hotel Tokyu | The Peninsula Tokyo |

---

## 7. Estimate — NET (Wholesale) per Person

> Japan land portion only · 10 nights · **3 travelers sharing one room (triple)** · **NET rates for you to mark up** (or commissionable 10–15% on request). Estimate, subject to final confirmation.

| Item | Value | **Standard (recommended)** | Luxury |
|---|---|---|---|
| Accommodation (10 nights, one room shared by 3) | ¥80,000 | ¥140,000 | ¥500,000 |
| Transport (rail · charters · transfers) | ¥60,000 | ¥95,000 | ¥230,000 |
| Experiences & culture program *(Nagoya menu at real rates — see §5)* | ¥30,000 | ¥85,000 | ¥150,000 |
| Guide · interpretation · on-site support | ¥18,000 | ¥45,000 | ¥60,000 |
| Selected meals (ryokan dinner, experience meals) | ¥3,000 | ¥10,000 | ¥10,000 |
| **TOTAL (NET / person)** | **¥190,000** | **¥375,000** | **¥950,000** |
| ≈ USD (at ¥150/USD) | **$1,270** | **$2,500** | **$6,350** |

> **Small-group pricing note:** with only 3 guests, fixed costs (guide, private transport) are higher per person, but sharing **one room among three** lowers lodging cost — so the per-person total lands close to a standard twin-share trip.
> **Triple-room note:** budget/standard hotels have limited triple inventory, so we book a **twin + extra bed or a Japanese-style room (3 futons)**. Some 5★ properties cannot place 3 in one room; a **suite or a second room** may be required at additional cost (to be confirmed).

**Includes:** 10 nights' hotels · domestic rail, private charters & airport transfers · listed experiences, entries & English-speaking guide · selected meals.
**Excludes:** international flights · Seoul stay · meals not listed · personal expenses · travel insurance · peak-season surcharges · anything not listed above.

---

## 8. Terms (draft)

- **Rates:** NET / wholesale (mark up freely) or 10–15% commissionable on request.
- **Currency:** JPY (USD shown for reference; rate applied at settlement).
- **Validity:** 30 days from issue (October is peak season — early confirmation advised).
- **Payment (example):** 30% deposit on confirmation · balance 30 days before arrival.
- **Cancellation (example):** free to 30 days · 30% (29–15) · 50% (14–8) · 100% (≤7 days).
- **Minimum group size:** flexible; per-person cost rises for smaller parties (from 2 pax for luxury).

---

## 9. To Finalize, Please Confirm

1. ~~Number of travelers & rooming~~ → **confirmed: 3 pax, one shared room** (please note if a 5★ stay must keep all 3 in one room)
2. **Target price grade** (Value / Standard / Luxury)
3. Japan departure airport (NRT/HND) & flight status
4. Whether we should also coordinate the **Seoul** portion
5. Dietary needs (vegetarian / halal / allergies)

Once confirmed, we will issue a **final itinerary + formal quotation (English PDF)**.

*Kensuke Ueoka — BUSHIDO JAPAN · ueoka@bsd-pro.com*
