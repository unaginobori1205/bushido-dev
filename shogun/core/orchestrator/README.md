...
`server.ts` is meant to run either locally (`pnpm dev:core`, loopback-only,
no auth needed) or on a cloud host (docs/DEPLOYMENT.md) — `auth.ts` (unit
tested, see `auth.test.ts`) is the chokepoint that refuses to bind to a
non-loopback address without a `CORE_AUTH_TOKEN`, and checks every
incoming WebSocket connection's token before `server.ts` does anything
else with it. See docs/ARCHITECTURE.md §11 and docs/SECURITY.md.
